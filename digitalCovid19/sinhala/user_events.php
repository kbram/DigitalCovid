...
<html lang="en-US">
<head>
  <title>DigitalCovid19 - සහභාහී වීම් </title>
...
<div class="container-fluid mt-5 pt-5 pb-5">
	<div class="row pt-4">
		...
		<div class="col-xs-12 col-sm-12 col-md-8 col-lg-9 fnt-gray pl-2 pr-2">
			<h2 class="fnt-green pl-1">සහභාහී වීම් පිලිබඳ තොරතුරු ඇතුලත් කිරීම</h2>
			<div class="loginbox pl-4 pr-4 pt-2">
				<form >
				<div class="form-group">
					<input required type="text" onchange="fillLocation(this.value)" name="eventName" id="eventName" class="form-control eventName" placeholder="සිදුවීම / නම">
				</div>
				<div class="form-group">
					<input required type="text" name="location" id="location" class="form-control" placeholder="ස්ථානය">
				</div>
				<div class="form-group">
					<label class="pl-2">දිනය</label>
					<input required type="date" id="eDate" name="eDate" max="3000-12-31" min="1000-01-01" class="form-control">
				</div>
				<div class="form-group text-left">
					<button type="button" name="save-submit" id="btn_saveform" class="btn btn-green" value="save-submit">ඇතුලත් කරන්න</button>
				</div>
			</form>
			</div><br>
			<h2 class="fnt-green pl-1">සියලු සහභාහී වීම්</h2><br>
...
<script type="text/javascript">
	...
	function addEvent(e){
		if(e.name==""){
			window.alert("ඇතුලත් කිරීම අසාර්ථකයි")
		}else{
			...
			window.alert("ඇතුලත් කිරීම සාර්ථකයි")
		}
		...
		arr.forEach(function(item){
			...
			EventsReference.on("value",function(snapshot){
				...
				snapshot.forEach(function(childsnapshot){
					...
					if(events.name==item){
						heroHTMLItems+="<div class='addedItems shadow-sm p-3 text-left'><p class='fnt-green'><i class='fas fa-calendar-alt'></i></p><p><small>"+events.date+"</small><br>"+events.place+"<br>"+events.name+"<br></p></div>";
						mess=1;
					}
				});
				if(mess==0){
					heroHTMLItems+="<p align='left'>කිසිදු සහභාගීවීමක් ඇතුලත් කර නොමැත. කරුණාකර තොරතුරු ඇතුලත් කරන්න</p><br><br>";
				}
			});
		});
