...
<html lang="en-US">
<head>
  <title>රෝග ලක්ෂණ - DigitalCovid19</title>
...
<div class="container-fluid mt-5 pt-5 pb-5">
	<div class="row pt-4">
		...
		<div  class="col-xs-12 col-sm-12 col-md-8 col-lg-9 fnt-gray pl-2 pr-2">
			...
			<h2 class="fnt-green">රෝග ලක්ෂණ වෙනස් කිරීමට</h2><br>
			<div class="loginbox pl-4 pr-4">
				<form>
				<div class="form-check">
				  <label class="form-check-label">
				    <input id="breathing" type="checkbox" class="form-check-input" value="breathing_difficulties">හුස්ම ගැනීමේ අපහසුතාව
				  </label>
				</div>
				<div class="form-check">
				  <label class="form-check-label">
				    <input id="losssmell" type="checkbox" class="form-check-input" value="losssmell">ගඳ සුවඳ නොදැනීම
				  </label>
				</div>
				<div class="form-check">
				  <label class="form-check-label">
				    <input id="ferver" type="checkbox" class="form-check-input" value="fever">උණ
				  </label>
				</div>
				<div class="form-check">
				  <label class="form-check-label">
				    <input id="headache" type="checkbox" class="form-check-input" value="headache">හිසරදය
				  </label>
				</div>
				<div class="form-check">
				  <label class="form-check-label">
				    <input id="cough" type="checkbox" class="form-check-input" value="cough">කැස්ස
				  </label>
				</div>
				<div class="form-check">
				  <label class="form-check-label">
				    <input id="sore_throat" type="checkbox" class="form-check-input" value="sore_throat">උගුරේ වේදනාව
				  </label>
				</div>
				<div class="form-group pt-3">
				<label>ඔබට වෙනත් රෝග තිබේද?</label>
				<br>
				<textarea id="DiseaseHistory" class="form-control" value="DiseaseHistory" placeholder="හෘද රෝග, අධික රුධිර පීඩනය වැනි වෙනත් රෝග"></textarea>
				</div>
				<div class="form-group pt-3">
					<label>ඔබ විදෙස්ගත වී තිබේද?</label><br>
					<div class="custom-control custom-radio custom-control-inline">
					    <input checked type="radio" class="custom-control-input" id="abroadid" name="abroad" value="abroad">
					    <label class="custom-control-label" for="abroadid">ඔව්</label>
					 </div>
					<div class="custom-control custom-radio custom-control-inline">
					    <input type="radio" class="custom-control-input" id="notabroadid" name="abroad" value="notAbroad">
					    <label class="custom-control-label" for="notabroadid">නැත</label>
					</div>
				</div>
				<div class="form-group">
			      <label for="country">කුමන රටද?</label>
				...
				<div class="form-group text-left">
					<button type="button" name="save-submit" id="btn_saveform" tabindex="5" class="btn btn-green" value="save-submit">ඇතුලත් කරන්න</button>
				</div>
			</form>
			</div>
		</div>
...
$("#btn_saveform").click(function(){
	...
	if((abroadFrom=="" ||abroadTo=="" ||country=="" )&& $('#abroadid').prop('checked')){
		...
		window.alert("දිනය නිවැරදිව ඇතුලත් කරන්න")
	}else{
		var symptoms ={
...
function addSymptom(d){
	if(nic==""){
		...
		window.alert("ඇතුලත් වීම සාර්ථකයි")
	}
}
...
symptomsReference.on("value",function(snapshot){
	$("#show-symptoms").empty();
	var heroHTMLItems="<h2 class='fnt-green'>වර්තමාන රෝග ලක්ෂණ</h2><br>";
	var mess=0;
	snapshot.forEach(function(childsnapshot){
		var symptom=childsnapshot.val();
		if(childsnapshot.key==nic){
			heroHTMLItems+="<p><b>රෝග ලක්ෂණ : </b>";
			if(symptom.breathin==true){
				heroHTMLItems+="හුස්ම ගැනීමේ අපහසුතාව |";
			}if(symptom.losssmell==true){
				heroHTMLItems+="ගඳ සුවඳ නොදැනීම |";
			}if(symptom.ferver==true){
				heroHTMLItems+=" උණ |";
			}if(symptom.headache==true){
				heroHTMLItems+="හිසරදය |";
			}if(symptom.cough==true){
				heroHTMLItems+="කැස්ස |";
			}if(symptom.throat==true){
				heroHTMLItems+="උගුරේ වේදනාව |";
			}
			if(symptom.DiseaseHistory){
				heroHTMLItems+="<br><strong>දැනට පවතින වෙනත් රෝග: </strong>"; 
				heroHTMLItems+=symptom.DiseaseHistory;
			}
			heroHTMLItems+="</p>";
			if(symptom.abroad==true){
				heroHTMLItems+="</p><p><b>විදෙස්ගත වීම් :</b>"+symptom.country +" ("+symptom.date+")</p><br><br>";
			}else{
				heroHTMLItems+="</p><p><b>විදෙස්ගත වීම් : </b>නැත</p><br><br>";
			}
			mess=1;
		}
	...
	if(mess==0){
			heroHTMLItems+="<p align='left'>රෝග ලක්ෂණ ඇතුලත් කර නොමැත</p><br><br>";
	}
